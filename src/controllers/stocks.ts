...
const getStocks = async (req: any, res: any) => {
  ...
  try {
    ...
    const updateQuery =
      "UPDATE users SET searchedSymbols = JSON_MERGE_PRESERVE(IFNULL(searchedSymbols, JSON_ARRAY()), JSON_ARRAY(?)) WHERE email = ?";
    await dbConnection.promise().query(updateQuery, [symbol, email]);

    const [stockRow]: any = await dbConnection
      .promise()
    ...
    if (existingStock) {
      ...
      const [userRow]: any = await dbConnection
        .promise()
    ...
    const [creatingStockRow]: any = await dbConnection
      .promise()
      .query("INSERT INTO stocks SET ?", {
        symbol: symbol,
...
const fetchStockDataForAllSymbols = async () => {
  ...
  try {
    const [usersRow]: any = await dbConnection
      .promise()
      ...
      if (searchedSymbols && searchedSymbols.length > 0) {
        for (const symbol of searchedSymbols) {
          ...
          if (!stocks) {
            ...
            const [updatedStocks] = await dbConnection
              .promise()
              .query("UPDATE stocks SET stockData = ? WHERE symbol = ?", [
